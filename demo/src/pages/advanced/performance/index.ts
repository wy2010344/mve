...
export default function () {
  ...
  const { performanceMetrics, generateTestData, performStressTest, clearData } =
      ...
      fdom.div({
        ...
        children() {
          const metricCards = [
            {
              label: '渲染次数',
              value: () => performanceMetrics.get().renderCount,
              icon: '🔄',
              color: 'blue',
            },
            {
              label: '渲染耗时',
              value: () => `${performanceMetrics.get().lastRenderTime}ms`,
              icon: '⏱️',
              color: 'green',
            },
            {
              label: 'memo 命中',
              value: () => performanceMetrics.get().memoHits,
              icon: '🎯',
              color: 'purple',
            },
            {
              label: 'memo 未命中',
              value: () => performanceMetrics.get().memoMisses,
              icon: '❌',
              color: 'red',
            },
          ];
      ...
      fdom.div({
        ...
        children() {
          fdom.button({
            ...
            className:
              'px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-lg font-medium hover:shadow-lg transition-all duration-200',
            childrenType: 'text',
            children: '🚀 生成测试数据',
          });
          ...
          fdom.button({
            ...
            className:
              'px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg font-medium hover:shadow-lg transition-all duration-200',
            childrenType: 'text',
            children: '💥 压力测试',
          });
