...
function useTrigger<E extends HTMLElement, V, F>(
  value: GetValue<F>,
  setValue: SetValue<F>,
  transform: StoreTransform<F, V>,
  ...
  const getTriggerTime = valueOrGetToGet(
    triggerTime || 'onInputWithComposition'
  );
...
export function renderInputTrans<T extends 'textarea' | 'input' | 'select', B>(
  ...
  useTrigger(
    getValue,
    setValue,
    transform || anyStoreTransform,
    new InputCache(div as HTMLInputElement),
    triggerTime
  );
  return div;
}
export function renderInput<T extends 'textarea' | 'input' | 'select'>(
  ...
  return renderInputTrans(
    anyStoreTransform as StoreTransform<string, string>,
    getValue,
    setValue,
    div,
    triggerTime
...
export function renderContentEditable<T extends DomElementType>(
  ...
  return renderContentEditableTrans(
    anyStoreTransform as StoreTransform<string | null, string | null>,
    getValue,
    setValue,
    div,
    triggerTime
...
export function renderContentEditableHtmlTrans<T extends DomElementType, B>(
  ...
  useTrigger(
    ...
    setValue,
    transform,
    new ContentHTMLCache(div),
    triggerTime
  );
  return div;
}

export function renderContentEditableHtml<T extends DomElementType>(
  getValue: GetValue<string>,
  setValue: SetValue<string>,
  div: DomElement<T>,
  triggerTime?: ValueOrGet<TriggerTime>
) {
  return renderContentEditableHtmlTrans(
    anyStoreTransform as StoreTransform<string, string>,
    getValue,
    setValue,
    div,
