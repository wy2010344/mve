...
export function hookDrawText(
  ...
  }) as LayoutNode<CMNode, PointKey> & {
    measure: GetValue<
      Omit<OCanvasTextDrawingStyles, "textAlign" | "textBaseline"> & {
        text: string;
      } & {
        textBaseline?: CanvasTextBaseline;
        measure: TextMetrics;
...
export function hookDrawTextWrap(
  arg: {
    ...
    /**只与绘制相关 */
    drawInfo?:
      | ((arg: MeasuredTextWrapOut) => DrawTextWrapExt)
      | DrawTextWrapExt;
    draw?(
      ctx: CanvaRenderCtx,
      n: LayoutNode<CMNode, PointKey>,
  ...
  const d = hookDrawRect({
    ...
    measure: GetValue<MeasuredTextWrapOut>;
  };
  const mout = memo(function () {
